  ...
  (cond [(var? e) 
        ...
        [(mlet? e)
         (let ([v-name (mlet-var e)]
               [v-exp (eval-under-env (mlet-e e) env)])
           (if (string? v-name)
               (eval-under-env (mlet-body e) (cons (cons v-name v-exp) env))
               (error "MUPL mlet var is non-string")))]
        [(call? e)
           ...
           (if (closure? clo)
               (letrec ([f (closure-fun clo)]
                 ...
                 (eval-under-env (fun-body f)
                                 (if (string? f-name)
                                     (cons (cons f-name clo) new-env)
                                     new-env)))
               (error "First call subexpression is not a MUPL closure")))]
        [(apair? e)
         (apair (eval-under-env (apair-e1 e) env) 
                (eval-under-env (apair-e2 e) env))]
        [(fst? e)
         (define x (eval-under-env (fst-e e) env))
         (if (apair? x)
             (apair-e1 x)
             (error "MUPL fst applied to non-pair"))]
        [(snd? e)
         (define x (eval-under-env (snd-e e) env))
         (if (apair? x)
             (apair-e2 x)
