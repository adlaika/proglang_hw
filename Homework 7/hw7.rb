...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2)
      ...
      if real_close(s1.y2, s2.y1) then Point.new(s1.x2, s1.y2)
      elsif s1.y2 < s2.y2 then NoPoints.new
      elsif s1.y2 > s2.y2 then LineSegment.new(s2.x1, s2.y1, s2.x2, s2.y2)
      else LineSegment.new(s2.x1, s2.y1, s1.x2, s1.y2)
      end
    else 
      s1 = @x1 < seg.x1 ? self : seg
      s2 = @x1 < seg.x2 ? seg : self
      if real_close(s1.x2, s2.x1) then Point.new(s1.x2, s1.y2)
      elsif s1.x2 < s2.x2 then NoPoints.new
      elsif s1.x2 > s2.x2 then LineSegment.new(s2.x1, s2.y1, s2.x2, s2.y2)
      else LineSegment.new(s2.x1, s2.y1, s1.x2, s1.y2)
      end
    end
  end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    new_env = Array.new(env).unshift([@s, @e1.eval_prog(env)])
