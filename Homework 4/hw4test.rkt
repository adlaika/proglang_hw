...
(define tests
  (test-suite
   ...
   (check-equal? (stream-for-n-steps (cycle-lists (list 1) (list "a" "b")) 3) (list (cons 1 "a") (cons 1 "b") (cons 1 "a")) 
    ...
    (let ([c1 (cached-assoc (list (cons 1 2) (cons 3 4) (cons 5 6)) 3)])
                        (check-equal? (c1 1) (cons 1 2) "CA test 2")
                        (check-equal? (c1 3) (cons 3 4) "CA test 3")
                        (check-equal? (c1 6) #f "CA test 4"))
